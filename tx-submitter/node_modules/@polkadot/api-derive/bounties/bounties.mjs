...
export function bounties(instanceId, api) {
  ...
  return memo(instanceId, () => combineLatest([bountyBase.bountyCount(), api.query.council.proposalCount()]).pipe(switchMap(() => combineLatest([bountyBase.bounties.keys(), api.derive.council.proposals()])), switchMap(([keys, proposals]) => {
    const ids = keys.map(({
      args: [id]
    }) => id);
