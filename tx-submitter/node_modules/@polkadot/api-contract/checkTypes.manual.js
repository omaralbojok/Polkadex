...
async function checkBlueprint(api, pairs) {
  const blueprint = new _.BlueprintPromise(api, _incrementer.default, '0x1234'); // new style
...
async function checkContract(api, pairs) {
  const contract = new _.ContractPromise(api, _incrementer.default, '0x1234'); // queries
  ...
  await contract.tx.inc(123, 456, 69).signAndSend(pairs.eve); // deprecated
